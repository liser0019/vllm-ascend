...
single TP rank. CPU copy is not graph-capturable and requires eager mode. The
`sparse_copy` backend requires the paired registered-host MemFabric build: it
registers the Host KV pool once and writes the resulting device-visible
addresses into both H2D and D2H descriptors. A plain CPU `Tensor.data_ptr()` is
never used as an AIV/MTE Host address in that backend.

To test NPU LRU management without executing the MemFabric `sparse_copy` kernel:
...
To submit the complete three-operator NPU path on a single rank:
...
This selects `lru_resident_compact`, `compute_lru_resident_addrs`, and
`sparse_copy`. The paired MemFabric package must expose
`OffloadConfig.register_host_memory` and `offload.get_device_address`; startup
fails explicitly if either interface is missing.
