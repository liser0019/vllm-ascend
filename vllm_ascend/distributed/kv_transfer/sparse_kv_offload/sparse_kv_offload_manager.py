...
class SparseKVOffloadManager:
    ...
    def __init__(
        ...
        self._runtime_evidence_logged: set[str] = set()
        ...
        if self.tp_size != 1 and (
            ...
            or self.copy_backend in (
                COPY_BACKEND_CPU,
                COPY_BACKEND_SPARSE_COPY,
            )
        ):
            raise RuntimeError(
                "The experimental NPU LRU and registered-host copy backends "
                "currently "
        ...
        if self.copy_backend == COPY_BACKEND_SPARSE_COPY:
            missing_ops = [
                name
                for name in ("sparse_copy", "get_device_address")
                if not hasattr(offload, name)
            ]
            if missing_ops:
                raise RuntimeError(
                    "The installed MemFabric package does not provide the "
                    "registered-host sparse copy interfaces: "
                    f"{', '.join(missing_ops)}"
                )
        ...
        if hasattr(config, "register_host_memory"):
            config.register_host_memory = False
        if self.copy_backend == COPY_BACKEND_SPARSE_COPY:
            if not hasattr(config, "register_host_memory"):
                raise RuntimeError(
                    "The installed MemFabric OffloadConfig does not support "
                    "register_host_memory. Build the paired registered-host "
                    "MemFabric package before enabling sparse_copy."
                )
            config.register_host_memory = True
        assert offload.initialize(config) == 0, "Sparse KV offload offload.initialize failed."
        self.tp_group.barrier()

    def _register_host_copy_address(self, tensor: torch.Tensor) -> int:
        """Resolve and retain the address consumed by the selected copy backend."""
        if tensor.device.type != "cpu" or not tensor.is_contiguous():
            raise ValueError(
                "Sparse KV offload Host pool tensors must be contiguous CPU tensors"
            )
        cpu_address = int(tensor.data_ptr())
        if self.copy_backend == COPY_BACKEND_SPARSE_COPY:
            copy_address = int(offload.get_device_address(tensor))
        else:
            copy_address = cpu_address
        if not 0 < copy_address <= MAX_SIGNED_INT64:
            raise RuntimeError(
                "MemFabric returned an invalid Host copy address: "
                f"{copy_address}"
            )
        self._host_copy_addresses[cpu_address] = copy_address
        return copy_address

    def _get_host_copy_address(self, tensor: torch.Tensor) -> int:
        cpu_address = int(tensor.data_ptr())
        try:
            return self._host_copy_addresses[cpu_address]
        except KeyError as exc:
            raise RuntimeError(
                "Sparse KV offload Host pool tensor was not registered with "
                "the copy backend"
            ) from exc

    def _log_runtime_evidence(self, stage: str, **fields: object) -> None:
        if stage in self._runtime_evidence_logged:
            return
        self._runtime_evidence_logged.add(stage)
        details = " ".join(f"{key}={value}" for key, value in fields.items())
        logger.warning("[KV_RUNTIME_EVIDENCE] %s %s", stage, details)
    ...
    def register_kv_caches(
        ...
        self._host_copy_addresses: dict[int, int] = {}
        ...
        if self.tp_rank == 0:
            for layer_id in range(self.num_layers):
                ...
                gvas_k_tensor[layer_id] = self._register_host_copy_address(k_cpu)
                gvas_v_tensor[layer_id] = self._register_host_copy_address(v_cpu)
    ...
    def offload_new_kv(
        ...
        dst_k_base = self._get_host_copy_address(k_cache_cpu)
        dst_v_base = self._get_host_copy_address(v_cache_cpu)
        ...
        if self.copy_backend == COPY_BACKEND_SPARSE_COPY:
            self._log_runtime_evidence(
                "sparse_copy_d2h",
                descriptors=2 * token_count,
                mapping="halHostRegister_dva",
            )
